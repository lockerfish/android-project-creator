...
{
  TITLE="Android Project Creator Script"
...
printHelp() {
  ...
  echo "-t 'Target API'"
  echo "-p 'package'"
  echo "-a 'Main Activity'"
}

# -----------------------------------------------------------------------------
# Get all options
# -----------------------------------------------------------------------------
while getopts ":h:n:t:p:a:" optname "$@"
  do
     case "$optname" in
       "h")
          printHelp
...
# -----------------------------------------------------------------------------
# Locate ANDROID SDK installation directory. It contains the gradle wrapper
...
# header note.
# 
sdk.dir=/Users/hendrix/Library/Android/sdk
...
$NAME is an awesome app. 
_EOF_
}

createProguardRulesFile() {
  cat << _EOF_ >app/proguard-rules.pro
...
createAppGradleBuildFile() {
  TEST_TEMP=`cat app/build.gradle | grep "compileSdkVersion"`
  TEST_TEMP=`echo ${TEST_TEMP//[[blank:]]/}`
  TEST_TEMP=`echo ${TEST_TEMP:18}`
  TEST_TEMP=`echo ${TEST_TEMP/android-/}`
  TEST_TEMP=`echo ${TEST_TEMP/\'/}`
  COMPILE_SDK_VERSION=`echo $TEST_TEMP | awk '{printf("%d",$1)}'`
  TEST_TEMP=`cat app/build.gradle | grep "buildToolsVersion"`
  TEST_TEMP=`echo ${TEST_TEMP//[[blank:]]/}`
  BUILD_TOOLS_VERSION=`echo ${TEST_TEMP:17}`
  cat << _EOF_ >app/build.gradle
...
if [[ -n "$SDK_DIR" && -n "$NAME" && -n "$TARGET" && -n "$PACKAGE"  
  && -n "$ACTIVITY" && -n "$APP" && -n "$GRADLE_PLUGIN_VERSION" ]]; then

  cp -rf $SDK_DIR/tools/templates/gradle/wrapper/gradlew .
  cp -rf $SDK_DIR/tools/templates/gradle/wrapper/gradlew.bat .
  cp -rf $SDK_DIR/tools/templates/gradle/wrapper/gradle .

  android create project -n $NAME -t $TARGET -k $PACKAGE -a $ACTIVITY -p $APP -g -v $GRADLE_PLUGIN_VERSION
  ...
  # sed -e 's/runProguard/minifyEnabled/' -e '1,8d' app/build.gradle > $TMP_FILE
